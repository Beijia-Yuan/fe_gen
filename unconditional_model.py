...
from helpers import validate_sequences
...
class seq_dataset(Dataset):
    # FIX: removed unused fes_vectors parameter
...
class SequenceDecoder(nn.Module):
    def __init__(self, d_model=320, nhead=8, num_layers=4, vocab_size=VOCAB_SIZE, max_len=64):
        ...
        # FIX: use TransformerEncoder (GPT-style) instead of TransformerDecoder,
        # since there is no encoder memory for an unconditional model.
        # TransformerDecoder requires a `memory` arg and would crash with it commented out.
    ...
    def forward(self, tgt_tokens, tgt_key_padding_mask=None):
        ...
        # FIX: TransformerEncoder uses `mask` and `src_key_padding_mask`
        out = self.decoder(
            src=x,
            mask=causal_mask,
...
class decoder_model(nn.Module):
    # FIX: removed unused ESM2 encoder — unconditional model needs no encoder
    def __init__(self, d_model=320, nhead=8, num_layers=4):
        super().__init__()
    ...
    def forward(self, labels=None):
        # FIX: removed `device` arg (derive from labels) and `encoded_fes` (undefined)
        loss = None
        logits = None
        if labels is not None:
            B = labels.shape[0]          # FIX: B was never defined
            device = labels.device       # FIX: derive device from the input tensor
            decoder_input = labels.clone()
            decoder_input[decoder_input == -100] = PAD_TOKEN
            decoder_input = torch.cat(
                [
                    torch.full((B, 1), EOS_TOKEN, dtype=torch.long, device=device),
                    decoder_input[:, :-1],
                ],
                dim=1,
            )
            pad_mask = decoder_input == PAD_TOKEN
            logits = self.decoder(decoder_input, tgt_key_padding_mask=pad_mask)
    ...
    @torch.no_grad()
    def generate(self, batch_size=32, max_len=42, temperature=1.0, device="cpu"):
        # FIX: `B` and `device` are now proper parameters, not free variables
        generated = torch.full(
            (batch_size, 1), EOS_TOKEN, dtype=torch.long, device=device
        )
...
def train(
    ...
    # FIX: removed `unilat_fes` param — unconditional model needs no FE vectors
    model.to(device)
    best_val_loss = 1e9
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        model.train()
        total_loss = 0
        for labels in tqdm(dataloader_train, desc="train"):
            labels = labels.to(device)
            optimizer.zero_grad()
            with torch.amp.autocast("cuda"):
                loss, _ = model(labels=labels)
        ...
        total_loss_val = 0
        with torch.no_grad():
            for labels in tqdm(dataloader_val, desc="val"):
                labels = labels.to(device)
                with torch.cuda.amp.autocast():
                    # FIX: removed undefined `fes` arg; forward() only needs labels
                    loss, _ = model(labels=labels)
                total_loss_val += loss.item()

        avg_train = total_loss / len(dataloader_train)
        avg_val = total_loss_val / len(dataloader_val)
        print(f"Epoch {epoch+1}/{n_epochs} | Train Loss: {avg_train:.4f} | Val Loss: {avg_val:.4f}")

        if avg_val < best_val_loss:
            best_val_loss = avg_val
            torch.save(model.state_dict(), "unconditional_decoder.pt")

        # --- inference sample ---
        model.eval()
        # FIX: generate no longer takes fes; pass batch_size and device explicitly
        generated = model.generate(batch_size=32, max_len=42, temperature=0.6, device=device)
        ...
        for i, seq in enumerate(generated):
            ...
            if SEP_TOKEN in seq:
                ...
                seq2 = [
                    idx_to_aa.get(t, "?")
                    for t in seq[split + 1 :]
                    if t not in (EOS_TOKEN, PAD_TOKEN)
                ]
            else:
...
#     def forward(self, device, labels=None):
#         loss = None
#         logits = None
#         if labels is not None:
#             decoder_input = labels.clone()
...
#     @torch.no_grad()
#     def generate(self, max_len=42, temperature=1.0, device=device):
...
#         model.train()
#         total_loss = 0
#         for labels in tqdm(dataloader_train):
#             labels = labels.to(device)
#             optimizer.zero_grad()
...
#         total_loss_val = 0
#         with torch.no_grad():
#             for labels in tqdm(dataloader_val):
#                 labels = labels.to(device)
#                 with torch.cuda.amp.autocast():
#                     loss, _ = model(fes, labels=labels, device=device)
#                 total_loss_val += loss.item()
...
#             torch.save(model.state_dict(), 'unconditional_esm2_fes_finetuned.pt')
